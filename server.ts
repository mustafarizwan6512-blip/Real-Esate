...
const db = {
  projects: [
    ...
    },
    {
      ...
      description: "A distinguished residential and commercial destination on Al Hamra Street, Jeddah developed by LAMER REAL ESTATE.",
      highlights: ["Prime Al Hamra Location", "Full Luxury Amenities", "Commercial & Residential Units"],
      amenities: ["Swimming Pool", "Gym", "Lounge", "Cinema Hall", "Meeting Facilities"],
      images: [
...
async function startServer() {
  ...
  // Redirect non-www and enforce HTTPS in production
  app.use((req, res, next) => {
    const host = req.headers.host || '';
    const proto = req.headers['x-forwarded-proto'] || req.protocol;

    if (process.env.NODE_ENV === 'production' && proto !== 'https') {
      return res.redirect(301, `https://www.referestates.com${req.url}`);
    }

    if (host === 'referestates.com') {
      return res.redirect(301, `https://www.referestates.com${req.url}`);
    }
