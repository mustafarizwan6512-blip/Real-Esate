...
// Helper to provide human-readable architectural and amenity labels for gallery photographs
function getImageCaption(project: Project | null, index: number, src?: string): string {
  const url = (src || '').toLowerCase();
  if (url.includes('intercom')) return 'Video Intercom Access';
  if (url.includes('amenities')) return 'Dedicated Amenity Spaces';
  if (url.includes('booking')) return 'Building Services Booking App';
  if (url.includes('smart-access')) return 'Smart Access For Units & Amenities';
  if (url.includes('pool')) return 'Rooftop Swimming Pool & Terrace';
  if (url.includes('cinema')) return 'Private Cinema Screening Hall';
  if (url.includes('lounge')) return 'Executive Lounge & Meeting Spaces';
  if (url.includes('gym')) return 'Fitness & Health Club';
  if (url.includes('facade')) return 'Architectural Facade & Exterior';
  if (url.includes('entrance')) return 'Grand Arrival & Reception';
  if (url.includes('living-room')) return 'Curated Luxury Living Spaces';
  if (url.includes('bedroom')) return 'Master Suite & Bedroom';
  if (url.includes('staircase')) return 'Grand Interior Staircase';
  if (url.includes('guarantees')) return 'Structural Guarantees & Specifications';
  if (url.includes('view')) return 'Panoramic City View';
  if (index === 0) return 'Primary Exterior & Facade';
  if (index === 1) return 'Curated Living & Amenity Spaces';
  return `${project?.name || 'Project'} - View ${index + 1}`;
}
...
export default function ProjectDetail() {
  ...
  const projectJsonLd = {
    ...
    "url": typeof window !== 'undefined' ? window.location.href : `https://www.referestates.com/projects/${project.slug || project.id}`,
    "offers": {
      "@type": "Offer",
  ...
  return (
    <div className="w-full bg-cream text-secondary">
      <SEO 
        ...
        canonical={`https://www.referestates.com/projects/${project.slug || project.id}`}
      ...
      <section className="relative min-h-[85vh] flex flex-col justify-between pt-28 pb-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 p-4 sm:p-5 bg-surface border border-secondary/10 shadow-xl">
            {/* Spec 1: Price */}
            <div className="space-y-1 border-r border-secondary/10 pr-3 sm:pr-4 last:border-0">
              <span className="block text-[10px] font-display font-bold uppercase tracking-[0.15em] text-primary">Starting Price</span>
              <p className="text-xs sm:text-sm font-display font-bold text-secondary break-words leading-tight">
            ...
            {/* Spec 2: Type */}
            <div className="space-y-1 border-r border-secondary/10 pr-3 sm:pr-4 last:border-0">
              <span className="block text-[10px] font-display font-bold uppercase tracking-[0.15em] text-primary">Property Type</span>
              <p className="text-xs sm:text-sm font-display font-bold text-secondary break-words leading-tight">
                {project.category || project.property_type || 'Residential, commercial'}
              </p>
            </div>

            {/* Spec 3: Status */}
            <div className="space-y-1 border-r border-secondary/10 pr-3 sm:pr-4 last:border-0">
              <span className="block text-[10px] font-display font-bold uppercase tracking-[0.15em] text-primary">Status</span>
              <p className="text-xs sm:text-sm font-display font-bold text-secondary break-words leading-tight">
            ...
            <div className="space-y-1">
              <span className="block text-[10px] font-display font-bold uppercase tracking-[0.15em] text-primary">Location</span>
              <p className="text-xs sm:text-sm font-display font-bold text-secondary break-words leading-tight">
      ...
      <section id="overview" className="py-24 sm:py-32 bg-cream border-b border-secondary/10 relative scroll-mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
            {project.location && (
              <div className="p-4 sm:p-5 bg-surface border border-secondary/10 hover:border-primary/40 transition-colors">
                <Compass className="w-5 h-5 text-primary mb-2.5" />
            ...
            {project.category && (
              <div className="p-4 sm:p-5 bg-surface border border-secondary/10 hover:border-primary/40 transition-colors">
                <Building2 className="w-5 h-5 text-primary mb-2.5" />
                <span className="block text-[10px] uppercase font-display font-bold tracking-widest text-secondary/60">Type</span>
                <p className="text-xs sm:text-sm font-display font-semibold text-secondary mt-1 break-words leading-snug">{project.category}</p>
              </div>
            )}

            {project.size && (
              <div className="p-4 sm:p-5 bg-surface border border-secondary/10 hover:border-primary/40 transition-colors">
                <Maximize2 className="w-5 h-5 text-primary mb-2.5" />
            ...
            {project.bedrooms && (
              <div className="p-4 sm:p-5 bg-surface border border-secondary/10 hover:border-primary/40 transition-colors">
                <Bed className="w-5 h-5 text-primary mb-2.5" />
                <span className="block text-[10px] uppercase font-display font-bold tracking-widest text-secondary/60">Bedrooms / Units</span>
                <p className="text-xs sm:text-sm font-display font-semibold text-secondary mt-1 break-words leading-snug">{project.bedrooms}</p>
              </div>
            )}

            {(project.status || project.handover_date) && (
              <div className="p-4 sm:p-5 bg-surface border border-secondary/10 hover:border-primary/40 transition-colors">
                <Sparkles className="w-5 h-5 text-primary mb-2.5" />
                <span className="block text-[10px] uppercase font-display font-bold tracking-widest text-secondary/60">STATUS</span>
                <p className="text-xs sm:text-sm font-display font-semibold text-secondary mt-1 break-words leading-snug">{project.status || project.handover_date}</p>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              {project.images[1] && (
                ...
                >
                  <img
                    ...
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-secondary/85 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-6">
                    <span className="text-xs uppercase tracking-widest text-cream font-display font-bold">
                      {getImageCaption(project, 1, project.images[1])}
                    </span>
                  </div>
                </div>
              )}
              ...
              {project.images.slice(2).map((img, idx) => (
                <div
                  ...
                  className="col-span-1 md:col-span-4 lg:col-span-3 group relative aspect-[4/3] overflow-hidden cursor-pointer bg-surface border border-secondary/10"
                >
                  <img
                    src={img}
                    alt={`${project.name} - ${getImageCaption(project, idx + 2, img)}`}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                    loading="lazy"
                    referrerPolicy="no-referrer"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-secondary/90 via-secondary/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-4">
                    <span className="self-end p-2 bg-secondary/80 text-cream">
                      <ZoomIn size={16} />
                    </span>
                    <span className="text-[11px] uppercase tracking-wider text-cream font-display font-semibold line-clamp-1">
                      {getImageCaption(project, idx + 2, img)}
                    </span>
                  </div>
                </div>
              ))}
          ...
          >
            ...
            <div className="flex items-center justify-between text-cream w-full z-10">
              <div className="space-y-0.5">
                <h4 className="text-sm font-display font-bold uppercase tracking-wider text-primary">
                  {project.name} &mdash; <span className="text-cream font-medium">{getImageCaption(project, activeImageIndex, project.images[activeImageIndex])}</span>
                </h4>
        ...
        <section className="py-24 bg-surface border-b border-secondary/10 relative">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mb-12 space-y-3">
              ...
              <p className="text-sm text-secondary/70 leading-relaxed font-body">
                {project.id === '3' || project.slug === 'lamer-elite-complex'
                  ? "The project’s location is directly on Al Hamra Street, linking King Abdulaziz Road (Al-Andalus) and Madinah Road, with proximity to Al Hamra Corniche, government entities, and modern tourist projects."
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {project.nearby_places.map((place, i) => (
                <div
                  key={i}
                  className="p-5 bg-cream border border-secondary/10 hover:border-primary/30 transition-colors flex flex-col justify-between"
                >
                  ...
                  {place.description && (
                    <p className="text-xs text-secondary/70 font-body leading-relaxed border-t border-secondary/10 pt-2.5 mt-1">
                      {place.description}
                    </p>
                  )}
                </div>
